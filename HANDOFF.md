...
- **進度**：**mentor-blueprint（10-80-10法則：讓AI變身超級實習生，專題設計效率翻倍）長片全流程交付完成**（本頻道女性人物基準照第一支影片）✅
  ...
  - **短片 1 支交付完成**（使用者事後追加要求，只做 1 支）✅：
    - **🔧 修復 `add_end_card.py` 舊 bug（全域＋專案已同步，日後所有短片受益）**：字卡背景解析度原本寫死 `1920x1080`，本支影片來源是 `1280x720`，concat 時尺寸不符直接失敗。已改成動態讀取來源影片的 `width`/`height`（`ffprobe` 取得），字卡與 fontsize 都按來源解析度等比例產生。
    - 亮點偵測 3 版候選（見 `working/mentor-blueprint/shorts-candidates.md`）：A 痛點型（66.7s，「現在學AI是不是太遲了」→10-80-10法則→「你是導演不是打字員」）／B 好奇型（65.4s，單一連續段，75%數據→AI剷平門檻→人類同理心創造力才是資產）／C 承諾型（47.1s，單一連續段，思維鏈+細化模式兩招）
    - 使用者選 **版本 C**：時間碼 `00:03:05.816-00:03:52.956`（原片單一連續段，未跨接）
    - 燒字幕後抽幀確認中文顯示正常；直式版抽幀確認內容置中不裁切、模糊背景填滿 1080×1920
    - 短片標題使用者選 #1：「AI回答廢話一堆？一句「思維鏈」秒破解」（3 個候選見 `working/mentor-blueprint/short-titles.md`）
    - 交付：`output/AI回答廢話一堆？一句「思維鏈」秒破解 [Claude] (Short)/`（7 檔齊全：乾淨版／字幕版／直式9x16／srt／txt／cover／metadata；封面橘色主題＋SHORT角標＋思維鏈鎖鏈視覺）
    - 未做的版本 A、B 時間碼已留存在 `shorts-candidates.md`，日後若要補剪可直接沿用
- **下一步**：等使用者下一步指令（可能是幫這支影片補其他版本短片、或丟下一支素材）
